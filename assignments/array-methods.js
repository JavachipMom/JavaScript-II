...
runners.forEach(runner => {
  return runnersLastName.push(runner.last_name);
})
...
companyNames.push(runners.filter(runner => {
  if (runner.company_name.includes("S")){
    return runner.company_name
  }
  return runner.company_name === "S"
}));
...
topDonators = runners.filter(donators => {
  if (donators.donation > 200){
    return true;
  } else {
    return false;
  }
...
console.log(topDonators.sort((a, b) => (b.donation - a.donation)));